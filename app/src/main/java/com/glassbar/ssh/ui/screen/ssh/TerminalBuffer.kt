...
/**
 * Snapshot of one screen's ring-buffer bookkeeping (main or alternate). Swapped into and out of
 * [TerminalBuffer]'s live fields when entering/leaving the alternate screen, so the existing
 * parser/grid logic keeps operating on plain instance fields without per-call indirection.
 */
private class ScreenState(
    var capacity: Int,
    var ringHead: Int,
    var ringRows: Array<Array<TerminalBufferCell>>,
    var screenTop: Int,
    var scrollTop: Int,
    var cursorRow: Int,
    var cursorCol: Int,
    var scrollMarginTop: Int,
    var scrollMarginBottom: Int,
    var savedCursorRelativeRow: Int,
...
class TerminalSnapshotRow internal constructor(
    private val cells: Array<TerminalBufferCell>,
) {
    val size: Int get() = cells.size
    operator fun get(index: Int): TerminalBufferCell = cells[index]
    internal fun copyCells(): Array<TerminalBufferCell> = cells.copyOf()
}
...
class TerminalBuffer(
    ...
    val cells: Array<Array<TerminalBufferCell>>
    ...
    /**
     * True when the live fields above (ring/cursor/margins/saved-cursor) represent the alternate
     * screen. The screen not currently live is parked in [inactiveScreenState].
     */
    @Volatile
    var isAlternateScreen: Boolean = false
        private set
    private var inactiveScreenState = createBlankScreenState(rows, cols, alternateScrollbackLines = 0)
     ...
     * Both the live screen and the parked (main-or-alternate) screen are resized together so
     * switching back after a resize during alternate-screen use sees consistent dimensions.
     */
    fun resize(newRows: Int, newCols: Int) {
        ...
        synchronized(lock) {
            ...
            val oldRows = rows
            val oldCols = cols
            val resizedLive = computeResizedScreen(
                currentScreenState(),
                oldRows,
                oldCols,
                targetRows,
            ...
            val resizedInactive = computeResizedScreen(
                inactiveScreenState,
                oldRows,
                oldCols,
                targetRows,
            ...
            rows = targetRows
            cols = targetCols
            applyScreenState(resizedLive)
            inactiveScreenState = resizedInactive
    ...
    /** Scrollback capacity of whichever screen is currently live. */
    private fun liveScrollbackLines(): Int = if (isAlternateScreen) 0 else scrollbackLines

    /** Scrollback capacity of the parked (not currently live) screen. */
    private fun inactiveScrollbackLines(): Int = if (isAlternateScreen) scrollbackLines else 0

    private fun currentScreenState(): ScreenState = ScreenState(
        capacity = capacity,
        ringHead = ringHead,
        ringRows = ringRows,
        screenTop = screenTop,
        ...
        savedCursorRelativeRow = savedCursorRelativeRow,
        savedCursorCol = savedCursorCol,
    )

    private fun applyScreenState(state: ScreenState) {
        capacity = state.capacity
        ringHead = state.ringHead
        ringRows = state.ringRows
        screenTop = state.screenTop
        scrollTop = state.scrollTop
        cursorRow = state.cursorRow
        cursorCol = state.cursorCol
        scrollMarginTop = state.scrollMarginTop
        scrollMarginBottom = state.scrollMarginBottom
        savedCursorRelativeRow = state.savedCursorRelativeRow
        savedCursorCol = state.savedCursorCol
    }

    /** Swaps the live screen fields with the parked [inactiveScreenState]. No-op if already there. */
    private fun switchScreen(toAlternate: Boolean) {
        if (toAlternate == isAlternateScreen) return
        val outgoing = currentScreenState()
        applyScreenState(inactiveScreenState)
        inactiveScreenState = outgoing
        isAlternateScreen = toAlternate
    }

    /** Clears the currently-live screen to blank cells and homes its cursor. */
    private fun clearActiveScreenLocked() {
        for (row in screenTop until screenTop + rows) clearRow(row)
        cursorRow = screenTop
        cursorCol = 0
    }

    /**
     * Applies xterm private-mode 47/1047/1049 semantics. [clearOnSwitch] clears the alternate
     * screen immediately after entering and immediately before leaving, matching xterm's
     * "always present a clean alternate screen" behavior (used for 1047/1049 but not plain 47).
     * [saveRestoreCursor] additionally saves the cursor before entering and restores it after
     * leaving, as required for mode 1049.
     */
    private fun setAlternateScreen(enabled: Boolean, clearOnSwitch: Boolean, saveRestoreCursor: Boolean) {
        if (enabled == isAlternateScreen) return
        if (enabled) {
            if (saveRestoreCursor) saveCursor()
            switchScreen(toAlternate = true)
            if (clearOnSwitch) clearActiveScreenLocked()
        } else {
            if (clearOnSwitch) clearActiveScreenLocked()
    ...
    private fun createBlankScreenState(rowsCount: Int, colsCount: Int, alternateScrollbackLines: Int): ScreenState {
        val stateCapacity = rowsCount + alternateScrollbackLines
        return ScreenState(
            capacity = stateCapacity,
            ringHead = 0,
            ringRows = Array(stateCapacity) { newBlankRow(colsCount) },
            screenTop = 0,
            scrollTop = 0,
            cursorRow = 0,
            cursorCol = 0,
            scrollMarginTop = 0,
            scrollMarginBottom = rowsCount - 1,
            savedCursorRelativeRow = 0,
            savedCursorCol = 0,
        )
    }

    /** Pure resize computation for one screen's ring buffer, shared by the live and parked screens. */
    private fun computeResizedScreen(
        state: ScreenState,
        oldRows: Int,
        oldCols: Int,
        targetRows: Int,
        targetCols: Int,
        screenScrollbackLines: Int,
    ): ScreenState {
        val oldScreenTop = state.screenTop
        val oldScrollTop = state.scrollTop
        val oldCursorRow = state.cursorRow
        ...
        val targetCapacity = targetRows + screenScrollbackLines
        val preservedStart = (preservedEnd - targetCapacity).coerceAtLeast(0)
        val preserved = ArrayList<Array<TerminalBufferCell>>(targetCapacity)

        for (logicalIndex in preservedStart until preservedEnd) {
            val oldLine = state.ringRows[(state.ringHead + logicalIndex) % state.capacity]
        ...
        val newScreenTop = (preserved.size - targetRows).coerceIn(0, screenScrollbackLines)
        val droppedRows = preservedStart

        return ScreenState(
            capacity = targetCapacity,
            ringHead = 0,
            ringRows = Array(targetCapacity) { index ->
                if (index < preserved.size) preserved[index] else newBlankRow(targetCols)
            },
            screenTop = newScreenTop,
            scrollTop = if (viewportWasAtBottom) {
                newScreenTop
            } else {
                (oldScrollTop - droppedRows).coerceIn(0, newScreenTop)
            },
            cursorRow = (oldCursorRow - droppedRows).coerceIn(newScreenTop, newScreenTop + targetRows - 1),
            cursorCol = state.cursorCol.coerceIn(0, targetCols - 1),
            scrollMarginTop = 0,
            scrollMarginBottom = targetRows - 1,
            savedCursorRelativeRow = state.savedCursorRelativeRow.coerceIn(0, targetRows - 1),
            savedCursorCol = state.savedCursorCol.coerceIn(0, targetCols - 1),
        )
    }
    ...
    /**
     * Feed decoded text; ASCII controls still pass through the ANSI state machine. Iterates full
     * Unicode code points (not UTF-16 code units) so surrogate-pair glyphs such as emoji are
     * placed as a single cell instead of two malformed ones. Non-ASCII code points are only
     * placed directly on the grid while the parser is in [EscapeState.NORMAL]; while a control
     * sequence (OSC/CSI/escape) is in progress, every code point — ASCII or not — must go through
     * the state machine instead of bypassing it and rendering into the grid mid-sequence.
     */
    fun write(text: String) {
        ...
        synchronized(lock) {
            ...
            while (index < text.length) {
                ...
                if (codePoint <= 0x7F || escapeState != EscapeState.NORMAL) {
                    processByte(codePoint)
                } else {
    ...
    fun visibleRows(): List<Array<TerminalBufferCell>> = snapshot().lines.map { it.copyCells() }
    ...
    fun textInRange(selection: TerminalSelection): String = synchronized(lock) {
        ...
        buildString {
            for (rowIndex in firstRow..lastRow) {
                ...
                while (trimmedEnd >= firstCol && (line[trimmedEnd].text == " " || line[trimmedEnd].wideContinuation)) {
                    trimmedEnd--
                }
                for (col in firstCol..trimmedEnd) {
                    if (!line[col].wideContinuation) append(line[col].text)
                }
    ...
    private fun processNormal(value: Int) {
        when (value) {
            ...
            0x0A -> lineFeed()
            0x0D -> cursorCol = 0
            in 0x20..0x7E, in 0xA0..0xFF -> putCodePoint(value)
        }
    }
    ...
    private fun processOscEscape(value: Int) {
        when (value) {
            '\\'.code -> finishOsc()
            // BEL is also a valid OSC terminator; treat it the same even right after a stray ESC
            // instead of appending both bytes as payload and leaving the parser stuck in OSC.
            0x07 -> finishOsc()
            else -> {
                // ESC not followed by ST or BEL is data in the OSC payload.
                appendOsc(0x1B)
                appendOsc(value)
                escapeState = EscapeState.OSC
            }
    ...
    /**
     * Appends one code point to the OSC payload. Uses [StringBuilder.appendCodePoint] (not
     * `Int.toChar()`, which silently truncates to the low 16 bits) so supplementary-plane code
     * points such as emoji survive intact instead of becoming an unrelated BMP character. Bounds
     * are measured in UTF-16 units a code point will actually occupy ([Character.charCount] is 1
     * or 2), so a surrogate pair straddling the limit is rejected as overflow instead of only
     * half-written.
     */
    private fun appendOsc(value: Int) {
        val unitCount = Character.charCount(value)
        if (oscString.length + unitCount > MAX_OSC_LENGTH) {
            oscOverflow = true
            return
        }
        if (value in 0..Character.MAX_CODE_POINT) {
            oscString.appendCodePoint(value)
        } else {
    ...
    /**
     * Places one Unicode code point. Combining marks (Mn/Mc/Me) merge into the glyph immediately
     * before the cursor instead of occupying their own cell or advancing the cursor. [char] keeps
     * BMP compatibility (first UTF-16 unit for non-BMP glyphs); [TerminalBufferCell.text] is the
     * authoritative full glyph and is always kept consistent with the placed code point(s).
     */
    private fun putCodePoint(codePoint: Int) {
        if (isCombiningCodePoint(codePoint)) {
            appendCombiningMark(codePoint)
            return
        }
        ...
        if (cursorCol in 0 until cols) {
            ...
            line[cursorCol] = TerminalBufferCell(
                char = glyph[0],
                text = glyph,
                fg = currentFg,
                bg = currentBg,
            ...
            if (charWidth == 2 && cursorCol + 1 < cols) {
                line[cursorCol + 1] = TerminalBufferCell(
                    fg = currentFg,
                    bg = currentBg,
                    bold = currentBold,
                    underline = currentUnderline,
    ...
    /** Appends a combining mark to the base glyph immediately before the cursor, if any. */
    private fun appendCombiningMark(codePoint: Int) {
        if (cursorCol <= 0 || cursorCol > cols) return
        val line = logicalRow(cursorRow)
        var baseCol = cursorCol - 1
        if (line[baseCol].wideContinuation) baseCol--
        if (baseCol < 0) return
        val mark = String(Character.toChars(codePoint))
        val base = line[baseCol]
        line[baseCol] = base.copy(text = base.text + mark)
    }

    private fun isCombiningCodePoint(codePoint: Int): Boolean = when (Character.getType(codePoint).toByte()) {
        Character.NON_SPACING_MARK, Character.COMBINING_SPACING_MARK, Character.ENCLOSING_MARK -> true
        else -> false
    }

    private fun isWideCodePoint(codePoint: Int): Boolean = when (codePoint) {
        in 0x1100..0x115F,
        in 0x2329..0x232A,
        in 0x2E80..0x303E,
        in 0x3041..0xA4CF,
    ...
    /** Width/consistency check for a stored cell, based on its authoritative [TerminalBufferCell.text]. */
    private fun isWideCell(cell: TerminalBufferCell): Boolean {
        if (cell.text.isEmpty()) return false
        return isWideCodePoint(cell.text.codePointAt(0))
    }
    ...
    private fun applyPrivateModes(enabled: Boolean) {
        for (mode in csiParams) {
            when (mode) {
                ...
                47 -> setAlternateScreen(enabled, clearOnSwitch = false, saveRestoreCursor = false)
                1047 -> setAlternateScreen(enabled, clearOnSwitch = true, saveRestoreCursor = false)
                1049 -> setAlternateScreen(enabled, clearOnSwitch = true, saveRestoreCursor = true)
            }
        }
    }
    ...
    private fun fullReset(clearScreen: Boolean = false) {
        ...
        if (clearScreen) {
            clearStorageLocked()
            inactiveScreenState = createBlankScreenState(rows, cols, alternateScrollbackLines = 0)
        }
    }
    ...
    private fun logicalRow(logicalIndex: Int): Array<TerminalBufferCell> {
    ...
    private fun setLogicalRow(logicalIndex: Int, row: Array<TerminalBufferCell>) {
        ringRows[(ringHead + logicalIndex) % capacity] = row
    }

    private fun clearRow(logicalIndex: Int) = clearRowArray(logicalRow(logicalIndex))

    private fun clearRowArray(row: Array<TerminalBufferCell>) {
    ...
    private fun clearCellOccupant(
        row: Array<TerminalBufferCell>,
        column: Int,
        blank: TerminalBufferCell = blankCell(),
    ) {
    ...
    private fun sanitizeWideCells(row: Array<TerminalBufferCell>, width: Int) {
    ...
    private fun blankCell(): TerminalBufferCell =
        if (currentBg == TerminalColors.DEFAULT_BG) DEFAULT_CELL else TerminalBufferCell(bg = currentBg)

    private fun newBlankRow(width: Int): Array<TerminalBufferCell> {
        val blank = blankCell()
    ...
    private companion object {
        val DEFAULT_CELL = TerminalBufferCell()
